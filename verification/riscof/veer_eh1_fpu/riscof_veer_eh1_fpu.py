...
class veer_eh1_fpu(pluginTemplate):
    __model__ = "veer_eh1_fpu"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(
            config['PATH'] if 'PATH' in config else "", "Vtb_top")
        ...
        self.gcc_prefix = config['GCC_PREFIX'] if 'GCC_PREFIX' in config else 'riscv32-unknown-elf'
    ...
    def initialise(self, suite, work_dir, archtest_env):
        ...
        # runTests function
        self.compile_cmd = self.gcc_prefix + '-gcc -march={0} \
        ...
        self.hexgen_cmd = self.gcc_prefix + '-objcopy -O verilog {0} {1}'

        self.sigdump_cmd = self.dut_exe + ' +SIGFILE={0} +CODEFILE={1}/{2}'
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        # for veer_eh1_fpu start building the '--isa' argument. the self.isa is dutnmae specific and may not be
        # useful for all DUTs
    ...
    def runTests(self, testList):
        ...
        for testname in testList:
            ...
            hex_cmd = self.hexgen_cmd.format(elf, hex_)
            ...
            if self.target_run:
                ...
                simcmd = self.sigdump_cmd.format(sig_file, test_dir, hex_)
            else:
                simcmd = 'echo "NO RUN"'

            # concatenate all commands that need to be executed within a make-target.
            execute = '@cd {0}; {1}; {2}; {3};'.format(
                testentry['work_dir'], cmd, hex_cmd, simcmd)
