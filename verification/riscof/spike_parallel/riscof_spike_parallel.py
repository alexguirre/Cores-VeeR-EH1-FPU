...
class spike_parallel(pluginTemplate):
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            if cgf_file is not None:
                execute += self.dut_exe + ' --log-commits --log dump --misaligned --isa={0} +signature={1} +signature-granularity=4 {2};'.format(self.isa, sig_file, elf)
            else:
                execute += self.dut_exe + ' --misaligned --isa={0} +signature={1} +signature-granularity=4 {2};'.format(self.isa, sig_file, elf)
